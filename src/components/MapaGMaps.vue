...
function cargarGoogleMaps() {
  ...
  if (_gmapsPromise) return _gmapsPromise   // ya hay una carga en curso

  _gmapsPromise = new Promise((resolve, reject) => {
    window.__gmapsReady = () => { delete window.__gmapsReady; resolve() }
    const s = document.createElement('script')
    s.src = `https://maps.googleapis.com/maps/api/js?key=${API_KEY}&libraries=marker,places&callback=__gmapsReady&loading=async`//checar esto despues si esta bien o esta mal
    s.async = true
    s.onerror = () => { _gmapsPromise = null; reject(new Error('No se pudo cargar Google Maps.')) }
